...
public class HandlersFactory {
    ...
    private HandlersFactory() {
        ...
        methodHandlerMap.put(Constants.HandlerType.AOI_SEARCH_TYPE, new AOISearchHandler());
    }
    ...
    public void dispatchMethodHandler(MethodCall call, MethodChannel.Result result) {
        ...
        switch (method) {
            ...
            case MethodID.BMFWeatherSearchMethodID.WEATHER_SEARCH:
                ...
                break;
            case MethodID.AOISearchMethodID.AOI_SEARCH:
                methodChannelHandler =
                        methodHandlerMap.get(Constants.HandlerType.AOI_SEARCH_TYPE);
                break;
            default:
                break;
        }
    ...
    public void destroy() {
        ...
        // 退出返回到后台，这里需置空，重新请求时需要重新初始化，否则检索模块会调用destroy。检索回调不会再次返回
        sInstance = null;
    }
}